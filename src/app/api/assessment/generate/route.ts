...
Return ONLY valid JSON array. No markdown.`;
        ...
        const callGroqWithRetry = async (
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    console.log("\n🤖 ============================================");
                    console.log("🤖 GENERATED WITH GROQ AI (Assessment Questions)");
...
// GET endpoint - No caching in current schema
export async function GET(request: NextRequest) {
    try {
        ...
        // No caching in current TalentPulse schema - always generate fresh
        return NextResponse.json({
            success: false,
